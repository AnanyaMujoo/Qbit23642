...
//    Stage junctionStop(){ return new Stage(new Main(() -> MecanumJunctionReactor2.stop = true), RobotPart.exitAlways()); }
    ...
    AutoModule BackwardFirst = new AutoModule(
            ...
            lift.stageLift(1.0, heightMode.getValue(HIGH)+3.5).attach(outtake.stageReadyEndAfter(0.5))
    );

    AutoModule Backward = new AutoModule(
            outtake.stageMiddle(0.2),
            lift.stageLift(1.0, heightMode.getValue(HIGH)+4.0).attach(outtake.stageReadyEndAfter(0.5))
    );
    ...
    public void define() {
        ...
        customNumber(5, i -> {
            switch (i){
                ...
                case 2: x = 1.0;  s = 1.5;  break;
                case 3: x = 1.6;  s = 2.0;  break;
                case 4: x = 2.3;  s = 2.5;   break;
            }
            // Move to pick
...
//        // Park
        customCase(() -> {
            ...
            addTimedWaypoint(0.7, 0.5, -4.0, 122, 0);
            addTimedSetpoint(1.0, 0.8, 2.0, -4.0, 78, 0);
        }, () -> {
