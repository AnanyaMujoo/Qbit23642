...
public class TwoOdometryOldTest extends TeleUnitTest {
    ...
    protected void start() {
        ...
        gph1.link(Button.Y, OnTurnOffEventHandler.class, () -> { mecanumOdometry.reset(); customMove = false;});
    }
    ...
    protected void loop() {
        ...
        log.show("Odometry Pose", mecanumOdometry);
    }
    ...
    private AutoModule moveHeading(double target){
        return new AutoModule(new Stage(
                ...
                new Exit(() -> Math.abs(mecanumOdometry.getHeading()-target) < 2),
                drive.returnPart()
        ));
    }

    private Pose movePower(Pose target){
        Pose error = target.getAdded(mecanumOdometry.getCurrentPose().getInverted());
        ...
        Vector powerVector = new Vector(xPow, yPow).getRotated(-mecanumOdometry.getHeading()).getScaled(1);
